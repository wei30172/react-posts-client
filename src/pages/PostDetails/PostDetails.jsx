...
import { getPost, getPostsBySearch } from '../../state/actions/actionCreators/postsActions'
        ...
        <SectionDiv>
          ...
          <RecommendedDiv>
            {recommendedPosts.map(({ title, name, message, likes, selectedFile, _id }) => (
              <div style={{ margin: '1rem', cursor: 'pointer' }} onClick={() => openPost(_id)} key={_id}>
                <Typography gutterBottom variant="h6">{title}</Typography>
                <Typography gutterBottom variant="subtitle2">{name}</Typography>
                <Typography gutterBottom variant="subtitle2">{message}</Typography>
                <Typography gutterBottom variant="subtitle1" color="secondary" >Likes: {likes.length}</Typography>
                <img alt={title} style={{ borderRadius: '10px' }} width="200px" src={selectedFile} />
              </div>
            ))}
          </RecommendedDiv>
        </SectionDiv>
